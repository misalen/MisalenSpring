...
/**
 * Interface for fetching metadata from databases. The dialect is configured
 * with a ConnectionProvider but is not required to actually use any
 * connections.
 * 
 * The metadata methods all returns Iterator and allows for more efficient and
 * partial reads for those databases that has "flakey" JDBC metadata
 * implementions.
 * 
...
public interface MetaDataDialect {
	 ...
	 * 
	 * @param info
	 *            a {@link ReverseEngineeringRuntimeInfo} to extract Connection
	 *            and SQLExceptionConverter and other runtime info
	 */
	public void configure(String user, String password, String url,String driverClassName) throws SQLException, ClassNotFoundException;
	 ...
	 * 
	 * @param iterator
	 *            an iterator returned from one of methods on this dialect
	 */
	 ...
	 *            name or null
	 * @return iterator with map elements that has "TABLE_NAME", "TABLE_SCHEMA",
	 *         "TABLE_CAT", "INDEX_NAME", "COLUMN_NAME", "NON_UNIQUE", "TYPE"
	 *         keys.
	 */
	...
	/**
	 * Return iterator over the exported foreign keys that mathces catalog,
	 * schema and table
	 * 
	 * @param catalog
	 *            name or null
	 ...
	 * @param name
	 * @return iterator with map elements that has "TABLE_NAME", "TABLE_SCHEMA",
	 *         "TABLE_CAT", "HIBERNATE_STRATEGY" (null if no possible to
	 *         determine strategy, otherwise return hibernate identifier
	 *         strategy name/classname)
	 */
