...
import org.misalen.core.wxpay.sdk.WechatPayConstants;

/**
 * Created by blaketang on 2017/6/16.
 */
public class WechatPayAutoDomain implements IWechatPayDomain {
    private WechatPayAutoDomain(){}
    private static class WxpayDomainHolder{
        private static IWechatPayDomain holder = new WechatPayAutoDomain();
    }
    ...
    public synchronized DomainInfo getDomain(final WechatPayConfig config) {
        DomainStatics primaryDomain = domainData.get(WechatPayConstants.DOMAIN_API);
        ...
        if(switchToAlternateDomainTime == 0){   //first switch
            ...
            return new DomainInfo(WechatPayConstants.DOMAIN_API2, false);
        }else if(now - switchToAlternateDomainTime < MIN_SWITCH_PRIMARY_MSEC){
            DomainStatics alternateDomain = domainData.get(WechatPayConstants.DOMAIN_API2);
