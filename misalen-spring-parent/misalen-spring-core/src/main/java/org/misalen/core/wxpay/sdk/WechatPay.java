...
import org.misalen.core.wxpay.model.WechatPayUnifiedParam;
import org.misalen.core.wxpay.sdk.WechatPayConstants.SignType;
...
public class WechatPay {
	...
	private WechatPayRequest wxPayRequest;
	...
	public WechatPay(final WechatPayConfig config) throws Exception {
		...
		this.wxPayRequest = new WechatPayRequest(config);
	}

	/**
	 * 向 Map 中添加 appid、mch_id、nonce_str、sign_type <br>
	...
	public Map<String, String> fillRequestData(Map<String, String> reqData) throws Exception {
		...
		reqData.put("nonce_str", WechatPayUtil.generateUUID());
	...
	public String requestWithoutCert(String urlSuffix, Map<String, String> reqData, int connectTimeoutMs,
		...
		String resp = this.wxPayRequest.requestWithoutCert(urlSuffix, msgUUID, reqBody, connectTimeoutMs,
				readTimeoutMs);
		return resp;
	}
	...
	public Map<String, String> processResponseXml(String xmlStr) throws Exception {
		...
		if (return_code.equals(WechatPayConstants.FAIL)) {
			return respData;
		} else if (return_code.equals(WechatPayConstants.SUCCESS)) {
	...
	public Map<String, String> microPay(Map<String, String> reqData, int connectTimeoutMs, int readTimeoutMs)
		...
		String url;
		if (config.useSandbox()) {
			url = WechatPayConstants.SANDBOX_MICROPAY_URL_SUFFIX;
		} else {
			url = WechatPayConstants.MICROPAY_URL_SUFFIX;
		}
	...
	public Map<String, String> microPayWithPos(Map<String, String> reqData, int connectTimeoutMs) throws Exception {
		...
		while (true) {
			startTimestampMs = WechatPayUtil.getCurrentTimestampMs();
						...
						if (resultCode.equals("SUCCESS")) {
							...
							if (errCode.equals("SYSTEMERROR") || errCode.equals("BANKERROR")
								...
								remainingTimeMs = remainingTimeMs
										- (int) (WechatPayUtil.getCurrentTimestampMs() - startTimestampMs);
	 ...
	 * @throws Exception
	 */
	public Map<String, String> unifiedOrder(WechatPayUnifiedParam reqData) throws Exception {
	 ...
	 * @throws Exception
	 */
	public Map<String, String> unifiedOrder(WechatPayUnifiedParam reqData, int connectTimeoutMs, int readTimeoutMs)
			throws Exception {
		reqData.validation();
		String url;
		if (config.useSandbox()) {
	...
	public Map<String, String> orderQuery(Map<String, String> reqData, int connectTimeoutMs, int readTimeoutMs)
		...
		String url;
		if (config.useSandbox()) {
			url = WechatPayConstants.SANDBOX_ORDERQUERY_URL_SUFFIX;
		} else {
			url = WechatPayConstants.ORDERQUERY_URL_SUFFIX;
		}
		String respXml = this.requestWithoutCert(url, this.fillRequestData(reqData), connectTimeoutMs, readTimeoutMs);
		return this.processResponseXml(respXml);
	}
	...
	public Map<String, String> closeOrder(Map<String, String> reqData, int connectTimeoutMs, int readTimeoutMs)
		...
		String url;
		if (config.useSandbox()) {
			url = WechatPayConstants.SANDBOX_CLOSEORDER_URL_SUFFIX;
		} else {
			url = WechatPayConstants.CLOSEORDER_URL_SUFFIX;
		}
		String respXml = this.requestWithoutCert(url, this.fillRequestData(reqData), connectTimeoutMs, readTimeoutMs);
		return this.processResponseXml(respXml);
	}
	...
	public Map<String, String> refundQuery(Map<String, String> reqData, int connectTimeoutMs, int readTimeoutMs)
		...
		String url;
		if (config.useSandbox()) {
			url = WechatPayConstants.SANDBOX_REFUNDQUERY_URL_SUFFIX;
		} else {
			url = WechatPayConstants.REFUNDQUERY_URL_SUFFIX;
		}
		String respXml = this.requestWithoutCert(url, this.fillRequestData(reqData), connectTimeoutMs, readTimeoutMs);
		return this.processResponseXml(respXml);
	}
	...
	public Map<String, String> downloadBill(Map<String, String> reqData, int connectTimeoutMs, int readTimeoutMs)
		...
		String url;
		if (config.useSandbox()) {
			url = WechatPayConstants.SANDBOX_DOWNLOADBILL_URL_SUFFIX;
		} else {
			url = WechatPayConstants.DOWNLOADBILL_URL_SUFFIX;
		}
		...
		if (respStr.indexOf("<") == 0) {
			ret = WechatPayUtil.xmlToMap(respStr);
		} else {
			// 正常返回csv数据
			ret = new HashMap<String, String>();
			ret.put("return_code", WechatPayConstants.SUCCESS);
	...
	public Map<String, String> shortUrl(Map<String, String> reqData, int connectTimeoutMs, int readTimeoutMs)
		...
		String url;
		if (config.useSandbox()) {
			url = WechatPayConstants.SANDBOX_SHORTURL_URL_SUFFIX;
		} else {
			url = WechatPayConstants.SHORTURL_URL_SUFFIX;
		}
		String respXml = this.requestWithoutCert(url, this.fillRequestData(reqData), connectTimeoutMs, readTimeoutMs);
		return this.processResponseXml(respXml);
	}
	...
	public Map<String, String> authCodeToOpenid(Map<String, String> reqData, int connectTimeoutMs, int readTimeoutMs)
		...
		String url;
		if (config.useSandbox()) {
			url = WechatPayConstants.SANDBOX_AUTHCODETOOPENID_URL_SUFFIX;
		} else {
			url = WechatPayConstants.AUTHCODETOOPENID_URL_SUFFIX;
		}
		String respXml = this.requestWithoutCert(url, this.fillRequestData(reqData), connectTimeoutMs, readTimeoutMs);
		return this.processResponseXml(respXml);
	}
